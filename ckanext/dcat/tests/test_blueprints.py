...
class TestEndpoints:
    ...
    def test_dataset_default(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=dataset["name"], _format="rdf")
    ...
    def test_dataset_xml(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=dataset["name"], _format="xml")
    ...
    def test_dataset_ttl(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=dataset["name"], _format="ttl")
    ...
    def test_dataset_n3(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=dataset["name"], _format="n3")
    ...
    def test_dataset_jsonld(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=dataset["name"], _format="jsonld")
    ...
    def test_dataset_not_found(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=str(uuid.uuid4()), _format="n3")
    ...
    def test_catalog_default(self, app):
        ...
        url = h.url_for("dcat.read_catalog", _format="rdf")
    ...
    def test_catalog_ttl(self, app):
        ...
        url = h.url_for("dcat.read_catalog", _format="ttl")
    ...
    def test_catalog_modified_date_wrong_date(self, app):
        ...
        url = h.url_for("dcat.read_catalog", _format="ttl", modified_since="wrong_date")
    ...
    def test_catalog_q_search(self, app):
        ...
        url = h.url_for("dcat.read_catalog", _format="ttl", q="Fïrst")
    ...
    def test_catalog_fq_filter(self, app):
        dataset1 = factories.Dataset(
            title="First dataset", tags=[{"name": "economy"}, {"name": "statistics"}]
        )
        dataset2 = factories.Dataset(title="Second dataset", tags=[{"name": "economy"}])
        dataset3 = factories.Dataset(
            title="Third dataset", tags=[{"name": "statistics"}]
        )

        url = h.url_for("dcat.read_catalog", _format="ttl", fq="tags:economy")
    ...
    def test_catalog_pagination(self, app):
        ...
        assert _sort_query_params(
            ...
            h.url_for("dcat.read_catalog", _format="rdf", page=1, _external=True)
        )

        assert _sort_query_params(
            self._object_value(g, pagination, HYDRA.nextPage)
        ) == _sort_query_params(
            h.url_for("dcat.read_catalog", _format="rdf", page=2, _external=True)
        )

        assert _sort_query_params(
            self._object_value(g, pagination, HYDRA.lastPage)
        ) == _sort_query_params(
            h.url_for("dcat.read_catalog", _format="rdf", page=2, _external=True)
        )
    ...
    def test_catalog_profiles_not_found(self, app):
        ...
        url = h.url_for("dcat.read_catalog", _format="jsonld", profiles="nope")
...
class TestAcceptHeader:
    ...
    def test_dataset_basic(self, app):
        ...
        url = h.url_for("dataset.read", id=dataset["name"])
    ...
    def test_dataset_multiple(self, app):
        ...
        headers = {
            "Accept": "text/csv; q=1.0, text/turtle; q=0.6, application/ld+json; q=0.3"
        }
...
@pytest.mark.skipif(
    p.toolkit.check_ckan_version(max_version="2.4.99"),
...
class TestTranslations:
    ...
    def test_labels_default(self, app):
        ...
        dataset = factories.Dataset(extras=[{"key": "version_notes", "value": "bla"}])
    ...
    def test_labels_translated(self, app):
        ...
        url = h.url_for("dataset.read", id=dataset["name"], locale="ca")
...
class TestCroissant:
    ...
    def test_croissant_metadata_endpoint(self, app):
        ...
        url = (
            h.url_for("dataset.read", id=dataset["name"]).rstrip("/")
            + "/croissant.jsonld"
        )
        ...
        assert croissant_dict["conformsTo"] == {
...
class TestDatasetSeries:
    def test_dataset_series_ttl(self, app):
        ...
        url = h.url_for("dcat.read_dataset", _id=dataset_series["name"], _format="ttl")
        ...
        url = h.url_for(
            "dcat.read_dataset", _id=datasets_in_series[1]["name"], _format="ttl"
        )
        ...
        assert [t for t in g.triples((dataset_ref, RDF.type, DCAT.Dataset))]

        assert [t for t in g.triples((dataset_ref, DCAT.inSeries, dataset_series_ref))]
